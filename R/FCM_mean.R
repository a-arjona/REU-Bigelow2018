...
setwd("C:/Files/Work/Bigelow/Data/") # the folder with the data

library("e1071")

load(file = "./DINEOF_2018_processed_data/CHLGLOB_DINEOF19982017.rdata") # load data from build climato
...
write.table(tbl,file = "./txt_files/fuzzy_cl.txt", sep = "\t", row.names = F,col.names = F,quote = FALSE)

# ---------- Fuzzy C Means for Annual Data ----------

years <- 1998:2017

load("./DINEOF_2018_processed_data/array_index.rdata")
...
for (yr in 1:length(years)) {
  ...
  filename <- paste("C:/Files/Work/Bigelow/Data/txt_files/", as.character(c_year), "FCM.txt", sep = "")
